...
func (s *flagsSource) Resync(ctx context.Context, pub ConfigPublisher, _ time.Time) ([]string, error) {
	keys := make([]string, 0, len(s.values))
	for flagKey, value := range s.values {
		if err := pub.PublishFlag(ctx, 1, flagKey, FlagPayload{Enabled: true, Value: value}); err != nil {
			return keys, err
		}
		keys = append(keys, FlagKey(1, flagKey))
	}
	return keys, nil
}
